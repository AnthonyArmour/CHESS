...
import random
import numpy as np
from tensorflow.python.ops.gen_array_ops import mirror_pad
...
class MCTS():
  def __init__(
    self, IactionSpace, action_space, Imirror, mirror, recursion_limit=10, iterations = 5000,
  ...
  def rollout(self, turn, count):
    ...
    if self.board.is_checkmate():
        val = 1
    # elif count >= self.recursion_limit:
    else:
      if turn:
