...
class Tools():
    ...
    def train_RoboChess_SGD(self, model, x_samples, labels, epochs, conv=True):
        ...
        model.model.fit(
            x=evaluateX, y=evaluateY, batch_size=96,
    ...
    def train_SingleNet(self, model, x_samples, labels, epochs):
        evalX, evalY, target, other = self.train_RoboChess_SGD(model, x_samples, labels, epochs)
        # print(evalX.shape)
    ...
    def create_TestModel(self, filters=None):
        init = self.get_ConvNet(9, filters)
        model = MyModel(
            init,
            196,
            self.get_class_split(196)
